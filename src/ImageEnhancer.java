 ...
 * by Shuyang Wu for CSE 373 Assignment 1, Autumn, 2016.
 * Section BF.
...
public class ImageEnhancer extends Component implements ActionListener {
    ...
    // declare two variables to hold instances of Undo/Redo stack class
    BufferedImageStack undoStack;
    BufferedImageStack redoStack;	
    BufferedImageStack undoSave;
    BufferedImageStack redoSave;
    ...
    public ImageEnhancer() {
        ...
        //  create empty stack instances for the Undo stack and the Redo stack
        undoStack = new BufferedImageStack();
        redoStack = new BufferedImageStack();
        undoSave = new BufferedImageStack();
        redoSave = new BufferedImageStack();
    ...
    public void actionPerformed(ActionEvent e){
        //  save the current buffered image for undoing and dispose of any redoable actions.
    	undoStack.push(biWorking);
    	System.out.println("The actionEvent is "+e); //useful when debugging.
    	if (e.getSource()==exitItem) { System.exit(0); }
    	if (e.getSource()==blurItem) { blur(); undoItem.setEnabled(true);redoItem.setEnabled(false);redoStack.clear();}
    	if (e.getSource()==sharpenItem) { sharpen(); undoItem.setEnabled(true);redoItem.setEnabled(false);redoStack.clear();}
    	if (e.getSource()==darkenItem) { darken(); undoItem.setEnabled(true);redoItem.setEnabled(false);redoStack.clear();}
    	if (e.getSource()==photoNegItem) { photoneg(); undoItem.setEnabled(true);redoItem.setEnabled(false);redoStack.clear();}
    	if (e.getSource()==thresholdItem) { threshold(); undoItem.setEnabled(true);redoItem.setEnabled(false);redoStack.clear();}
    	if (e.getSource()==undoItem){
    		redoItem.setEnabled(true);
    		redoStack.push(biWorking);
    		try {
    			redoSave.push(undoStack.pop());
    			biFiltered = undoSave.pop();
    	...
    	if (e.getSource()==redoItem) {
    		...
    		undoStack.push(biWorking);
    		try {
    			undoSave.push(redoStack.pop());
    			biFiltered = redoSave.pop();
    		} catch (Exception exception) {
    			System.out.println(exception);
    		}
    ...
    public void run() {
        JFrame f = new JFrame("ImageEnhancer with Undo or Redo"); 
